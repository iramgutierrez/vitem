...
<div ng-app="packages" >
	...
	<div class="adv-table" ng-controller="PacksController" ng-init="$root.generateAuthPermissions({{ \Auth::user()->role_id; }});" >
		<div class="panel">
			<div class="panel-body">
				<div ng-view></div>
				<header class="panel-heading col-sm-12">
				<h1 class="col-sm-3">Paquetes</h1>
					<a ng-if="$root.auth_permissions.create.pack" href="{{ route('packs.create') }}"><button type="button" class="pull-right btn btn-success ">Agregar paquete</button></a>
				</header>    
...
<tbody ng-if="viewGrid == 'list'" >
	<tr class="gradeX" ng-repeat="pack in packsP | orderBy:sort:reverse">
		...
		<td>
			<a href="@{{ pack.url_show }}" ng-if="$root.auth_permissions.read.pack" >
				<button type="button" class="col-sm-3 btn btn-success"><i class="fa fa-eye"></i></button>
			</a>
			<a href="@{{ pack.url_edit }}" ng-if="$root.auth_permissions.update.pack">
				<button  type="button" class="col-sm-3 col-sm-offset-1 btn btn-info "><i class="fa fa-refresh"></i></button>
			</a>
			<a data-toggle="modal" href="#myModal@{{pack.id}}" ng-if="$root.auth_permissions.delete.pack">
				<button type="button" class="col-sm-3 col-sm-offset-1 btn btn-danger"><i class="fa fa-trash-o"></i></button>    
			</a>
			<div ng-if="$root.auth_permissions.delete.pack" class="modal fade" id="myModal@{{pack.id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="media">
						...
						<div class="media-body col-sm-8">
							...
							<div class="col-sm-6 pull-right">
								...
								<a href="@{{ pack.url_show }}" class="col-sm-4" ng-if="$root.auth_permissions.read.pack">
									<button type="button" class=" btn btn-success"><i class="fa fa-eye"></i></button>
								</a>
								<a href="@{{ pack.url_edit }}" class="col-sm-4" ng-if="$root.auth_permissions.update.pack">
									<button  type="button" class="col-sm-offset-1 btn btn-info "><i class="fa fa-refresh"></i></button>
								</a>
								<a data-toggle="modal" href="#myModalD@{{pack.id}}" class="col-sm-4" ng-if="$root.auth_permissions.delete.pack">
									<button type="button" class="col-sm-offset-1 btn btn-danger"><i class="fa fa-trash-o"></i></button>    
								</a>
								<div ng-if="$root.auth_permissions.delete.pack" class="modal fade" id="myModalD@{{pack.id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
