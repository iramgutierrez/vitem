...
@section('content')
<div   ng-app="expenses" >
<div class="adv-table" ng-controller="ExpensesController" ng-init="$root.generateAuthPermissions({{ Auth::user()->role_id }})">
<div class="panel">
    <div class="panel-body">
        <div ng-view></div>
        <header class="panel-heading col-sm-12">
            <h1 class="col-sm-3">Gastos</h1>
            <a ng-if="$root.auth_permissions.create.expense" href="{{ route('expenses.create') }}"><button type="button" class="pull-right btn btn-success ">Agregar gasto</button></a>
        </header>
        ...
        <table  class="display table table-bordered table-striped col-sm-12" id="dynamic-table" >
            <thead>
                <tr >
                    ...
                    <th class="col-sm-2" ng-show="$root.auth_permissions.read.user" >
            ...
            <tbody ng-if="viewGrid == 'list'" >
                <tr class="gradeX" ng-repeat="expense in expensesP | orderBy:sort:reverse">
                    ...
                    <td>
                        <a href="@{{ expense.url_show }}" ng-if="$root.auth_permissions.read.expense" >
                            <button type="button" class="col-sm-3 btn btn-success"><i class="fa fa-eye"></i></button>
                        </a>
                        <a href="@{{ expense.url_edit }}" ng-if="$root.auth_permissions.update.expense"  >
                            <button  type="button" class="col-sm-3 col-sm-offset-1 btn btn-info "><i class="fa fa-refresh"></i></button>
                        </a>
                        <a data-toggle="modal" href="#myModal@{{expense.id}}" ng-if="$root.auth_permissions.delete.expense"  >
                            <button type="button" class="col-sm-3 col-sm-offset-1 btn btn-danger"><i class="fa fa-trash-o"></i></button>    
                        </a>
                              <div ng-if="$root.auth_permissions.delete.expense" class="modal fade" id="myModal@{{expense.id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                              ...
                              <div class="media-body">
                                  ...
                                  <address>
                                      ...
                                      <strong>Tipo: @{{ expense.expense_type.name }}</strong><br>
                                        <span ng-if="$root.auth_permissions.read.user && expense.employee.user.name != ''">Empleado: &nbsp;@{{ expense.employee.user.name }}<br></span>
                                  ...
                                  <a href="@{{ expense.url_show }}" class="col-sm-4" ng-if="$root.auth_permissions.read.expense" >
                            <button type="button" class=" btn btn-success"><i class="fa fa-eye"></i></button>
                        </a>
                        <a href="@{{ expense.url_edit }}" class="col-sm-4" ng-if="$root.auth_permissions.update.expense" >
                            <button  type="button" class="col-sm-offset-1 btn btn-info "><i class="fa fa-refresh"></i></button>
                        </a>
                        <a data-toggle="modal" href="#myModalD@{{expense.id}}" class="col-sm-4" ng-if="$root.auth_permissions.delete.expense" >
                            <button type="button" class="col-sm-offset-1 btn btn-danger"><i class="fa fa-trash-o"></i></button>    
                        </a>
                              <div ng-if="$root.auth_permissions.delete.expense" class="modal fade" id="myModalD@{{expense.id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                  <div class="modal-dialog">
                                      <div class="modal-content">
                                          <div class="modal-header">
                                              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
